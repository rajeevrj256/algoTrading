...
@Repository
public interface FnoTradeLogRepository extends JpaRepository<FnoTradeLogEntity, Long> {

    List<FnoTradeLogEntity> findByTradeDate(LocalDate tradeDate);

    /** Max POS-n sequence in fno_trade_log — merged with trade_log so IDs stay unique. */
    @Query(value = "SELECT COALESCE(MAX(CAST(SUBSTRING(position_id FROM 'POS-([0-9]+)') AS INTEGER)), 0) " +
            "FROM fno_trade_log WHERE position_id ~ '^POS-[0-9]+$'", nativeQuery = true)
    Integer findMaxPositionSequence();

    /** Per-strategy realized performance over the last :days days (same shape as trade_log's). */
    @Query(value =
